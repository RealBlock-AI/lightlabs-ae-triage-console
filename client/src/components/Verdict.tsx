...
function Branch({ branch }: { branch: VerdictBranch }) {
  return (
    <div className="flex flex-1 flex-col gap-1 px-3.5 py-3">
      <p className="data text-[10px] uppercase tracking-[.13em] text-[#8a968f]">{branch.basisLabel}</p>
      <p className="font-serif text-[26px] font-semibold leading-[1.05] tracking-tight text-[#13261f]">{branch.verdict}</p>
      <p className="data text-[11px] text-[#67746e]">{branch.delta}</p>
      <p className="data text-[11px] text-[#67746e]">{branch.context}</p>
    </div>
  );
}
...
export default function Verdict({ headerNote, branches, decidingVariable, decidingValues, reason }: VerdictProps) {
  return (
    <section className="border border-[#2b3531]" aria-label="Verdict">
      {/* Header band */}
      <div className="flex items-center justify-between gap-3 border-b border-[#2b3531] bg-[#e9ede8] px-3.5 py-2">
        <p className="data text-[10px] uppercase tracking-[.13em] text-[#3d4841]">Verdict · not resolved</p>
      ...
      <div className="flex items-stretch">
        ...
        <div className="flex w-[82px] shrink-0 flex-col items-center justify-center gap-1 border-x-[1.4px] border-[#2b3531] bg-[#e9ede8] px-1.5 py-3 text-center">
          <p className="data text-[9px] uppercase tracking-[.12em] text-[#8a968f]">Deciding</p>
          <p className="data text-[11px] leading-[1.2] text-[#13261f]">{decidingVariable}</p>
      ...
      <div className="lane-escalate border-t border-[#2b3531] px-3.5 py-2">
