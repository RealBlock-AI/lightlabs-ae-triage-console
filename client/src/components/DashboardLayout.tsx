import { Activity, BarChart3, PlugZap, ShieldCheck } from "lucide-react";
import { useLocation } from "wouter";

export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  const [location, setLocation] = useLocation();
  const items = [{ icon: Activity, label: "AE Queue", path: "/" }, { icon: PlugZap, label: "HubSpot", path: "/integrations/hubspot" }, { icon: BarChart3, label: "Capacity", path: "/capacity" }];
