import { Activity, BarChart3, ClipboardPenLine, Network, PlugZap, ShieldCheck } from "lucide-react";
import { useLocation } from "wouter";

export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  const [location, setLocation] = useLocation();
  const items = [{ icon: Activity, label: "Canonical Queue", path: "/" }, { icon: Network, label: "Architecture", path: "/architecture" }, { icon: ClipboardPenLine, label: "Intake", path: "/intake" }, { icon: BarChart3, label: "Capacity", path: "/capacity" }, { icon: PlugZap, label: "Integrations", path: "/integrations" }];
