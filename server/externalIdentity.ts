...
export async function getContactBySlackUser(input: { workspaceId: string; slackUserId: string }) {
  ...
  if (canonical.status === "verified" && canonical.user) {
    ...
    const confirmation = binding ? { delivery_key: `account-binding:${binding.bindingId}`, text: `Your Slack account is now linked to ${account?.name ?? "your Light Labs account"}. You can ask Bobby for support in any shared customer channel.`, binding_id: binding.bindingId } : null;
    return { status: "verified" as const, workspace_id: input.workspaceId, slack_user_id: input.slackUserId, user_id: canonical.user.id, contact: { id: contact?.id ?? null, account_id: primaryMembership?.account.id ?? null, name: canonical.user.name }, candidate: null, internal_owner_user_id: primaryMembership?.membership.internalOwnerUserId ?? null, link_confirmation: { linked: true, status: "bound", binding_id: binding?.bindingId ?? null, account_id: account?.id ?? primaryMembership?.account.id ?? null, account_name: account?.name ?? null, owner_id: account?.ownerId ?? null, next_dm: confirmation } };
  }
