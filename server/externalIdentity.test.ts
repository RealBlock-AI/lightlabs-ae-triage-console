...
describe("external Slack identity candidates", () => {
  it("keeps a first-contact bridge sender pending until exact-email mapping, then reconciles only the linked interaction", async () => {
    ...
    const db = await getDb(); await resolveExternalSlackIdentityCandidate({ workspaceId, slackUserId, userId: 9001, accountId: "acct_northwind", resolvedByUserId: "usr_admin" });
    ...
    expect(candidate).toMatchObject({ status: "mapped", resolvedContactId: "con_northwind_ops", resolvedByUserId: "usr_admin" }); expect(interaction).toMatchObject({ contactId: "con_northwind_ops", accountId: "acct_northwind", ownerId: "usr_sarah", requestingUserId: 9001 }); expect(interaction?.lane).toBe("escalate");
