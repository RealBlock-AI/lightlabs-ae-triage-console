...
import { slackMcpIdentityRequests, teamMembers } from "../drizzle/schema";
...
export async function listInternalTeamMembers() {
  const db = await getDb(); if (!db) return [];
  return db.select({ id: teamMembers.id, name: teamMembers.name, email: teamMembers.email, role: teamMembers.role, slackWorkspaceId: teamMembers.slackWorkspaceId, slackUserId: teamMembers.slackUserId }).from(teamMembers);
}
...
export async function approveMcpIdentityRequest(input: { requestId: string; teamMemberId: string }) {
  ...
  const occupied = (await db.select().from(teamMembers).where(and(eq(teamMembers.slackWorkspaceId, request.slackWorkspaceId), eq(teamMembers.slackUserId, request.slackUserId))).limit(1))[0];
  if (occupied && occupied.id !== member.id) throw new Error("This Slack identity is already assigned to a different internal team member.");
