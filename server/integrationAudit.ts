...
export async function recordIntegrationAudit(event: { surface: "slack_ingest" | "mcp" | "bobby"; eventType: string; outcome: "accepted" | "rejected" | "error"; statusCode: number; slackWorkspaceId?: string | null; slackUserId?: string | null; method?: string | null; toolName?: string | null; interactionId?: string | null; metadata?: Record<string, unknown> }) {
...
export async function listIntegrationAudit(surface?: "slack_ingest" | "mcp" | "bobby") { const db = await getDb(); if (!db) return []; const query = db.select().from(integrationAuditEvents).orderBy(desc(integrationAuditEvents.createdAt)).limit(25); return surface ? db.select().from(integrationAuditEvents).where(eq(integrationAuditEvents.surface, surface)).orderBy(desc(integrationAuditEvents.createdAt)).limit(25) : query; }
